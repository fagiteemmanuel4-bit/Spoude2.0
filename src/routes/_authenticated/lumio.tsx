...
const PRIMARY: Tile[] = [
  ...
  { to: "/billing", label: "Plan & credits", desc: "Manage your AI credits and Pro plan.", icon: Zap, accent: "gold" },
];
...
function ActivityRow({
  ...
  const cls = "flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-sidebar-accent/60 transition-colors";
  const inner = (
    <>
  ...
  if (href.kind === "set") {
    return <Link to="/sets/$id" params={{ id: href.id }} className={cls}>{inner}</Link>;
  }
